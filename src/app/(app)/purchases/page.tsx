...
import {
  Plus, Search, CheckCircle, Loader2, Truck, AlertCircle,
} from "lucide-react";
...
export default function PurchasesPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: p }, { data: prods }] = await Promise.all([
      supabase.from("purchases").select("*").order("purchase_date", { ascending: false }).order("created_at", { ascending: false }),
      supabase.from("products").select("product_id, name, unit").eq("is_active", true).order("name"),
    ]);
    setPurchases(p ?? []);
    setProducts(prods ?? []);
    setLoading(false);
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {[
          { label: "Total Purchases", value: purchases.reduce((s, p) => s + p.total_amount, 0), accent: "" },
          { label: "Paid (COGS)", value: purchases.filter((p) => p.payment_status === "Paid").reduce((s, p) => s + p.total_amount, 0), accent: "text-primary" },
          { label: "Pending (AP)", value: totalPending, accent: "text-destructive" },
        ].map((c) => (
          <Card key={c.label}>
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground">{c.label}</p>
              <p className={cn("mt-1 text-xl font-semibold tabular-nums", c.accent)}>{formatINR(c.value)}</p>
            </CardContent>
          </Card>
        ))}
      </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 space-y-1.5">
                ...
                <Select value={watch("payment_status")} onValueChange={(v) => setValue("payment_status", v as any, { shouldValidate: true })}>
                  ...
                  <SelectContent>
                    <SelectItem value="Paid">Paid — stock will be added immediately</SelectItem>
                    <SelectItem value="Pending">Pending — Accounts Payable (stock added on payment)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
